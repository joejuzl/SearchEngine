...
public class Indexer {
	...
	public Indexer(String _url, RecordManager recman) throws IOException
	{
		//initialize
		url = _url;
	...
	public boolean parseURL() throws IOException{
		...
		document = Jsoup.connect(url).timeout(0).get();
			...
			System.out.println("URL: "+url+" could not be indexed");
			return false;
		}
...
//		System.out.println("Body: " +body.text().replaceAll("[^\\p{L}\\p{N}]", " ") );
		...
		titlekw=filterkw(titlekw,false);
		bodykw=filterkw(bodykw,true);
